...
export const EMPTY_ELEMENTS: Array<string> = ['area', 'base', 'br', 'col', 'embed', 'hr', 'img', 'input', 'keygen', 'link', 'menuitem', 'meta', 'param', 'source', 'track', 'wbr'];
...
export interface IHTMLTagProvider {
    getId(): string;
    isApplicable(languageId: string): any;
...
export class HTMLTagSpecification {
    public constructor(public label: string, public attributes: Array<string> = []) { }
}
...
function collectAttributesDefault(tag: string, collector: (attribute: string, type: string) => void, tagSet: ITagSet, globalAttributes: Array<string>): void {
...
function collectValuesDefault(tag: string, attribute: string, collector: (value: string) => void, tagSet: ITagSet, globalAttributes: Array<string>, valueSets: IValueSets, customTags?: { [tag: string]: Array<string> }): void {
    let prefix = attribute + ':';
    let processAttributes = (attributes: Array<string>) => {
        attributes.forEach(attr => {
    ...
    if (customTags) {
        let customTagAttributes = customTags[tag];
